...
const REPO = process.env.GITHUB_REPO || 'metro-vice';
...
async function ensureRepo(token) {
  ...
  if (check.status !== 200) {
    const create = await api(token, 'POST', '/user/repos', {
      name: REPO,
      description: 'Metro Vice — open-world GTA-style browser game',
      private: false,
      auto_init: true
    });
  ...
  if (ref.status === 409) {
    await api(token, 'PUT', `/repos/${OWNER}/${REPO}/contents/README.md`, {
      message: 'Initialize Metro Vice',
      content: Buffer.from('# Metro Vice\n\nOpen-world city action game.\n').toString('base64')
    });
...
(async () => {
  console.log(`🚀 Metro Vice Sync — "${MESSAGE}"`);
